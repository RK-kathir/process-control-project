...
socketio = SocketIO(
    ...
    async_mode='eventlet',
    ping_timeout=5,     # If a client goes silent for 5 seconds, drop them instantly
    ping_interval=2     # Check the connection every 2 seconds
)
...
def run_tuning(km, tm, taum, mode, overshoot, robust, metric,
    ...
    if order >= 2 and sopdt_keys:
        # Try SOPDT-specific rules first, then fall back to FOPDT
